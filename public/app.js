...
const App = window.App = {
  ...
  initMobileSidebar() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        // close any open modal (order/invoice/proof/promo/voucher/shortcuts/lightbox)
        document.querySelectorAll('[id$="Modal"], [id$="Wrap"], #appLightbox').forEach(el => {
  ...
  openMobileSidebar() {
    ...
    const btn = document.querySelector('#sidebarToggleBtn, .sidebar-toggle-btn');
  ...
  // Command Palette (Ctrl+K) — Navigasi & Pintasan Flagship
  _cmdItems: [],
  _activeCmdIdx: 0,
  ...
  openCommandPalette() {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div class="cmd-palette-box" onclick="event.stopPropagation()">
          <div class="cmd-palette-header">
            ...
            <input type="text" id="cmdPaletteInput" class="cmd-palette-input" placeholder="${esc(this.lang === 'en' ? 'Type a command or navigate (e.g. orders, theme)...' : 'Ketik perintah atau navigasi (mis. pesanan, tema, layanan)...')}" autocomplete="off" spellcheck="false">
            <kbd class="cmd-palette-kbd">ESC</kbd>
          </div>
          <div id="cmdPaletteList" class="cmd-palette-list"></div>
          <div class="cmd-palette-footer">
            <span>Navigasi: <kbd class="cmd-palette-kbd">↑</kbd> <kbd class="cmd-palette-kbd">↓</kbd> Pilih: <kbd class="cmd-palette-kbd">↵</kbd></span>
            <span>Embun SaaS</span>
          </div>
        </div>
      `;
      ...
      const inp = modal.querySelector('#cmdPaletteInput');
      inp.addEventListener('input', (e) => {
        this.renderCommandPaletteList(e.target.value);
      });

      inp.addEventListener('keydown', (e) => {
        const items = modal.querySelectorAll('.cmd-palette-item');
        if (items.length === 0) return;
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          this._activeCmdIdx = (this._activeCmdIdx + 1) % items.length;
          this._updateCmdActiveItem(items);
        } else if (e.key === 'ArrowUp') {
          e.preventDefault();
          this._activeCmdIdx = (this._activeCmdIdx - 1 + items.length) % items.length;
          this._updateCmdActiveItem(items);
        } else if (e.key === 'Enter') {
          e.preventDefault();
          const activeEl = items[this._activeCmdIdx];
  ...
  _updateCmdActiveItem(items) {
    items.forEach((el, idx) => {
      const active = idx === this._activeCmdIdx;
  ...
  getCommandItems() {
    const isStaff = this.user && ['Admin', 'Owner', 'Staff'].includes(this.user.role || this.user.user_role);
    const isEn = this.lang === 'en';
    const items = [
      // Navigation
      { id: 'nav_dashboard', group: isEn ? 'Navigation' : 'Navigasi', title: this.t('menu_dashboard'), desc: isEn ? 'Overview of laundry metrics' : 'Ringkasan performa laundry', icon: '📊', action: () => this.navigate('/dashboard') },
      { id: 'nav_orders', group: isEn ? 'Navigation' : 'Navigasi', title: this.t('menu_orders'), desc: isEn ? 'View and manage all orders' : 'Daftar semua pesanan masuk', icon: '🧺', action: () => this.navigate('/pesanan') },
      ...(isStaff ? [
        { id: 'nav_services', group: isEn ? 'Navigation' : 'Navigasi', title: this.t('menu_services'), desc: isEn ? 'Manage packages & pricing' : 'Kelola paket & tarif cuci', icon: '🧼', action: () => this.navigate('/layanan') },
        { id: 'nav_delivery', group: isEn ? 'Navigation' : 'Navigasi', title: this.t('menu_delivery'), desc: isEn ? 'Manage couriers & pickups' : 'Manajemen kurir & antar jemput', icon: '🚚', action: () => this.navigate('/delivery') },
        { id: 'nav_customers', group: isEn ? 'Navigation' : 'Navigasi', title: this.t('menu_customers'), desc: isEn ? 'Customer database' : 'Daftar data pelanggan', icon: '👥', action: () => this.navigate('/pelanggan') },
      ] : []),
      ...
      // Quick Actions
      { id: 'act_new_order', group: isEn ? 'Quick Actions' : 'Aksi Cepat', title: this.t('new_order'), desc: isEn ? 'Open new order cashier modal' : 'Buka modal input pesanan baru', icon: '➕', action: () => { this.navigate('/pesanan'); setTimeout(() => { const btn = document.getElementById('openNewOrderModal'); if (btn) btn.click(); }, 150); } },
  ...
  renderCommandPaletteList(query = '') {
    const listEl = document.getElementById('cmdPaletteList');
    if (!listEl) return;
    const q = (query || '').trim().toLowerCase();
    let items = this.getCommandItems();
    ...
    if (q) {
      ...
      items.push({
        id: 'search_query_' + q,
        group: isEn ? 'Search' : 'Pencarian',
        title: (isEn ? 'Search Orders for: ' : 'Cari Pesanan: ') + `"${q}"`,
        desc: isEn ? 'Filter orders list with this keyword' : 'Buka dan filter tabel pesanan dengan kata kunci ini',
        icon: '🔍',
        action: () => {
          this.navigate('/pesanan');
          setTimeout(() => {
            const inp = document.getElementById('ordSearch');
            if (inp) {
              inp.value = q;
              const btn = document.getElementById('btnFilterOrders');
              if (btn) btn.click();
            }
          }, 200);
        }
    ...
    if (items.length === 0) {
      listEl.innerHTML = `
        <div style="padding: 24px 16px; text-align: center; color: var(--muted); font-size: 13px;">
          <span>Tidak ada perintah atau halaman yang cocok dengan "<strong>${esc(query)}</strong>"</span>
        </div>
      `;
      this._cmdItems = [];
      this._activeCmdIdx = -1;
      return;
    }

    this._cmdItems = items;
    this._activeCmdIdx = 0;

    const groups = {};
    items.forEach((it, idx) => {
      if (!groups[it.group]) groups[it.group] = [];
      groups[it.group].push({ ...it, globalIdx: idx });
    });

    let html = '';
    for (const [groupName, groupItems] of Object.entries(groups)) {
      html += `<div class="cmd-group-title">${esc(groupName)}</div>`;
      groupItems.forEach(it => {
        const isActive = it.globalIdx === this._activeCmdIdx;
        html += `
          <div class="cmd-palette-item ${isActive ? 'active' : ''}" data-idx="${it.globalIdx}">
            <span class="cmd-palette-item-icon">${it.icon}</span>
            <div class="cmd-palette-item-text">
              <span style="font-weight: 600;">${esc(it.title)}</span>
              <span class="cmd-palette-item-desc">${esc(it.desc)}</span>
            </div>
          </div>
        `;
      });
    ...
    listEl.querySelectorAll('.cmd-palette-item').forEach(el => {
      ...
      el.addEventListener('mouseenter', () => {
        this._activeCmdIdx = parseInt(el.getAttribute('data-idx'), 10);
        this._updateCmdActiveItem(listEl.querySelectorAll('.cmd-palette-item'));
      });
    });
  },
  ...
  openLightbox(src, caption = '') {
    let lb = document.getElementById('appLightbox');
    if (!lb) {
    ...
    lb.innerHTML = `
      <div style="position:relative;max-width:90vw;max-height:90vh;display:flex;flex-direction:column;align-items:center;cursor:default;" onclick="event.stopPropagation()">
        <button type="button" aria-label="Tutup" onclick="document.getElementById('appLightbox').style.display='none'" style="position:absolute;top:-40px;right:0;border:none;background:rgba(255,255,255,0.2);color:#fff;border-radius:99px;width:32px;height:32px;display:grid;place-items:center;cursor:pointer;font-size:16px;font-weight:700;">✕</button>
        <img src="${src}" alt="Bukti Pembayaran Penuh" style="max-width:88vw;max-height:80vh;object-fit:contain;border-radius:8px;box-shadow:0 20px 40px rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.15);">
        ${caption ? `<div style="color:#ffffff;font-size:13px;font-weight:600;margin-top:12px;text-align:center;text-shadow:0 1px 2px rgba(0,0,0,0.8);">${esc(caption)}</div>` : ''}
      </div>
    `;
  ...
  openShortcutsModal() {
    ...
    const isEn = this.lang === 'en';
    const shortcuts = [
      { key: 'Ctrl + K / ⌘K', desc: isEn ? 'Open Command Palette' : 'Buka Command Palette & Navigasi Cepat' },
      { key: 'Esc', desc: isEn ? 'Close any modal or menu' : 'Tutup modal aktif atau menu navigasi' },
      { key: '?', desc: isEn ? 'Show keyboard shortcuts' : 'Buka dialog bantuan pintasan keyboard ini' },
      { key: 'Enter', desc: isEn ? 'Execute selected command' : 'Jalankan perintah terpilih di Command Palette' },
      { key: '↑ / ↓', desc: isEn ? 'Navigate list items' : 'Navigasi pilihan atas / bawah' }
    ];

    modal.innerHTML = `
      <div class="card" style="border-radius:var(--radius-md);padding:24px;max-width:440px;width:100%;margin:20px auto;box-shadow:var(--shadow-card);">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;border-bottom:1px solid var(--line);padding-bottom:12px;">
          <div style="display:flex;align-items:center;gap:8px;">
            <span style="font-size:18px;">⌨️</span>
            <h4 style="margin:0;font-size:16px;font-weight:800;color:var(--text);">${isEn ? 'Keyboard Shortcuts' : 'Pintasan Keyboard'}</h4>
          </div>
          <button type="button" aria-label="Tutup" onclick="document.getElementById('shortcutsModal').style.display='none'" style="border:none;background:var(--bg);color:var(--text);border-radius:6px;padding:6px 10px;cursor:pointer;font-weight:700;">✕</button>
        </div>
        ...
        <div class="card" style="width: 100%; max-width: 400px; background: var(--card); border: 1px solid var(--line); border-radius: var(--radius-lg); padding: 28px; color: var(--text);">
          ...
          <form id="loginForm">
            ...
            <div style="display: flex; justify-content: flex-end; margin-bottom: 16px;">
              <a href="/auth/lupasandi.html" style="color: var(--blue); font-size: 12px; text-decoration: none; font-weight: 600;">Lupa kata sandi?</a>
            </div>
          ...
          <div style="margin-top: 14px; text-align: center; border-top: 1px solid var(--line); padding-top: 12px;">
            <a href="/" style="color: var(--muted); font-size: 12px; text-decoration: none;">← Kembali ke Beranda</a>
          </div>
        </div>
      </div>
    `;
  ...
  renderApp() {
          ...
          <div class="topbar">
            <div class="topbar-inner" style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
              ...
              <div style="margin-left: auto; display: flex; align-items: center; gap: 10px;">
                <button id="cmdPaletteBtn" class="btn btn-ghost cmd-palette-trigger" type="button" aria-label="Command Palette (Ctrl+K)" title="Command Palette (Ctrl+K)" style="padding: 6px 12px; font-size: 12px; border-radius: 8px; border: 1px solid var(--line); background: var(--card); color: var(--muted); cursor: pointer; display: inline-flex; align-items: center; gap: 6px;">
                  <span>🔍</span> <span style="font-size: 12px; font-weight: 600;">Cari...</span> <kbd style="font-size: 10px; padding: 2px 5px; border-radius: 4px; background: var(--bg); border: 1px solid var(--line); font-family: inherit; font-weight: 700;">Ctrl K</kbd>
                </button>
                ${isStaff ? `
    ...
    document.addEventListener('click', async (e) => {
      ...
      if (e.target.id === 'btnBatchApply') {
        await this._applyBatchStatus();
      }
      if (e.target.id === 'btnBatchCancel') {
        document.querySelectorAll('.chk-order').forEach(cb => cb.checked = false);
        const chkAll = document.getElementById('chkAllOrders');
    ...
    document.addEventListener('submit', async (e) => {
      ...
      if (e.target.id === 'customerForm') {
        ...
        const payload = {
          action: id ? 'update_customer' : 'create_customer',
          full_name: name,
          phone: phone,
          address: address
        };
        ...
        try {
          ...
          if (data.ok) {
            ...
            if (modal) modal.style.display = 'none';
            this.toast(id ? 'Data pelanggan diperbarui' : 'Pelanggan baru berhasil ditambahkan', 'success');
            this.renderPelanggan();
          } else {
            this.toast(data.msg || 'Gagal menyimpan data pelanggan', 'error');
          }
        } catch {
    ...
    document.addEventListener('change', async (e) => {
      // Order status dropdown change (Optimistic UI Update)
      const statusSelect = e.target.closest('.status-select');
      if (statusSelect) {
        const id = Number(statusSelect.getAttribute('data-id'));
        const newStatus = statusSelect.value;
        const prevStatus = statusSelect.getAttribute('data-prev') || 'baru';
        ...
        if (row) {
          if (newStatus === 'selesai') {
            ...
            row.style.borderLeft = '3px solid var(--green, #10b981)';
          } else {
        ...
        try {
          ...
          const data = await res.json();
          if (data.ok) {
            statusSelect.setAttribute('data-prev', newStatus);
      ...
      // Batch select: individual checkbox
      if (e.target.classList.contains('chk-order')) {
        this._updateBatchBar();
      }
      // Batch select: select all
      if (e.target.id === 'chkAllOrders') {
        const checked = e.target.checked;
        document.querySelectorAll('.chk-order').forEach(cb => cb.checked = checked);
        this._updateBatchBar();
      }
      if (taskStatusSelect) {
    ...
    try {
      ...
      c.innerHTML = `
        <div class="filter-bar glass-panel">
          ...
          ${isStaff ? `<button class="btn" id="btnExportOrders" onclick="App.exportOrdersCsv()" style="padding: 8px 16px; font-size: 13px; background: var(--green, #10b981); color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">📥 CSV</button>` : ''}
          <button class="btn btn-primary" id="openNewOrderModal" style="${!isStaff ? 'display: none;' : ''}">+ Pesanan Baru</button>
        </div>

        ${isStaff && orders.length > 0 ? `
        <div id="batchBar" style="display:none; margin-bottom: 12px; padding: 10px 16px; background: var(--blue-soft); border: 1px solid var(--blue-border); border-radius: var(--radius-md); align-items: center; gap: 12px; font-size: 13px;">
          <span id="batchCount" style="font-weight: 700; color: var(--blue);">0 dipilih</span>
          <select id="batchStatus" class="input-control" style="padding: 4px 10px; font-size: 12px; width: auto;">
          ...
          <button class="btn btn-primary" id="btnBatchApply" style="padding: 6px 14px; font-size: 12px;">Terapkan</button>
          <button class="btn" id="btnBatchCancel" style="padding: 6px 14px; font-size: 12px; background: transparent; border: 1px solid var(--line); color: var(--muted);">Batal</button>
        </div>
        ` : ''}
        ...
        <div class="card glass-panel" style="padding: 24px; border-radius: var(--radius-md); overflow-x: auto;">
          <table class="table" style="width: 100%; table-layout: fixed; border-collapse: collapse; text-align: left; font-size: 13px;">
            ...
            <thead>
              <tr style="border-bottom: 2px solid var(--line); color: var(--muted);">
                ${isStaff ? `<th style="padding: 12px 6px;"><input type="checkbox" id="chkAllOrders" title="Pilih Semua"></th>` : ''}
            ...
            <tbody id="ordersTableBody">
              ${orders.length === 0 ? `
                <tr>
                  <td colspan="${isStaff ? 8 : 7}" style="padding: 0; border: none;">
                    ${this.renderEmptyState({
                      icon: '🧺',
              ...
              ` : orders.map(o => {
                ...
                <tr style="${rowStyle}">
                  ...
                  <td style="padding: 12px 10px;">
                    ${isStaff ? `
                      <select class="status-select input-control" data-id="${esc(o.id)}" data-prev="${esc(o.status)}" style="padding: 4px 8px; font-size: 12px; font-weight: 600;">
                  ...
                  <td style="padding: 12px 10px; text-align: right; white-space: nowrap;">
                    <div class="action-btn-group">
                      ...
                      ${isSelesai ? `
                        <button type="button" class="action-btn" onclick="App.openDeliveryModalForOrder('${esc(o.order_code)}', '${esc(o.customer_name)}', '${esc(o.customer_phone)}', '${esc(o.customer_address || '')}')" title="Jadwalkan Pengantaran Kurir" style="color:var(--green);font-weight:700;">🚚 Antar</button>
                      ` : ''}
            ...
            ${orders.length > 0 ? `
            <tfoot>
              <tr style="border-top: 2px solid var(--line); font-weight: 700; color: var(--text); font-size: 13px;">
                ${isStaff ? '<td></td>' : ''}
                <td style="padding: 12px 10px;" colspan="3">Total: ${orders.length} pesanan</td>
                <td style="padding: 12px 10px;">${orders.reduce((s, o) => s + Number(o.weight_kg || 0), 0).toFixed(1)} kg</td>
                <td style="padding: 12px 10px;">Rp ${orders.reduce((s, o) => s + Number(o.total_amount || 0), 0).toLocaleString('id-ID')}</td>
                <td colspan="2"></td>
              </tr>
            </tfoot>
            ` : ''}
  ...
  async renderPelanggan() {
    ...
    const c = document.getElementById('mainContent');
    const isStaff = ['Admin', 'Owner', 'Staff'].includes(this.user?.role || this.user?.user_role);
    c.innerHTML = this.renderSkeletonTable({ columns: isStaff ? 8 : 7, rows: 6, hasActions: isStaff });
      ...
      c.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
          ...
          <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
            <input type="text" id="custSearchInput" class="input-control" placeholder="🔍 Cari nama, kode, atau no. HP..." style="width: 240px; padding: 8px 14px; font-size: 13px;">
            <button class="btn" onclick="App.exportCustomersCsv()" style="padding: 8px 16px; font-size: 13px; background: var(--green, #10b981); color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">📥 CSV</button>
            ${isStaff ? `<button class="btn btn-primary" onclick="App.openCustomerModal()" style="padding: 8px 16px; font-size: 13px; font-weight: 600;">+ Pelanggan Baru</button>` : ''}
          </div>
        </div>
          ...
          <table class="table" id="customersTable" style="width: 100%; table-layout: fixed; border-collapse: collapse; text-align: left; font-size: 13px;">
            ...
            <thead>
              <tr style="border-bottom: 2px solid var(--line); color: var(--muted);">
                ...
                ${isStaff ? `<th style="padding: 12px 10px; text-align: right;">Aksi</th>` : ''}
              </tr>
            </thead>
                ...
                return `
                  <tr class="cust-row" data-search="${esc(String(cust.code || '') + ' ' + String(cust.full_name || '') + ' ' + rawPhone).toLowerCase()}" style="border-bottom: 1px solid var(--line); transition: background 0.15s ease;">
                    ...
                    ${isStaff ? `
                      <td style="padding: 12px 10px; text-align: right; white-space: nowrap;">
                        <div class="action-btn-group">
                          <button type="button" class="action-btn" onclick="App.openCustomerModal(${cust.id})" title="Edit Data Pelanggan">✏️ Edit</button>
                          <button type="button" class="action-btn action-btn-danger" onclick="App.deleteCustomer(${cust.id}, '${esc(cust.full_name)}')" title="Hapus Pelanggan">🗑️</button>
                        </div>
                      </td>
                    ` : ''}
                  </tr>
        ...
        <div id="customerModal" class="modal-backdrop" style="display: none;" role="dialog" aria-modal="true" aria-labelledby="custModalTitle">
          <div class="modal-dialog">
            <div class="modal-header">
              ...
              <button type="button" class="modal-close" onclick="document.getElementById('customerModal').style.display='none'" aria-label="Tutup">✕</button>
            </div>
            <form id="customerForm">
              <input type="hidden" id="custFormId" value="">
              <div style="margin-bottom: 14px;">
                <label style="display: block; font-size: 13px; font-weight: 600; margin-bottom: 4px; color: var(--text);">Nama Lengkap <span style="color:var(--red)">*</span></label>
                <input type="text" id="custFormName" class="input-control" required style="width: 100%;" placeholder="Nama lengkap pelanggan">
              </div>
              <div style="margin-bottom: 14px;">
                ...
                <input type="text" id="custFormPhone" class="input-control" required style="width: 100%;" placeholder="08xxxxxxxxxx">
              </div>
              <div style="margin-bottom: 18px;">
                <label style="display: block; font-size: 13px; font-weight: 600; margin-bottom: 4px; color: var(--text);">Alamat</label>
                <textarea id="custFormAddress" class="input-control" rows="3" style="width: 100%; resize: vertical;" placeholder="Alamat penjemputan / pengantaran"></textarea>
              </div>
              <div style="display: flex; justify-content: flex-end; gap: 10px;">
                <button type="button" class="btn" onclick="document.getElementById('customerModal').style.display='none'" style="padding: 8px 18px;">Batal</button>
                <button type="submit" class="btn btn-primary" style="padding: 8px 22px;">Simpan Pelanggan</button>
              </div>
            </form>
          </div>
        </div>
  ...
  openCustomerModal(id = null) {
    ...
    if (id) {
      if (title) title.textContent = '✏️ Edit Data Pelanggan';
      const cust = (this._customers || []).find(c => Number(c.id) === Number(id));
      if (cust) {
        document.getElementById('custFormName').value = cust.full_name || '';
  ...
  async deleteCustomer(id, name) {
    if (!await this.confirm(`Hapus pelanggan "${name}"? Riwayat pesanan lama tetap tersimpan.`)) return;
    try {
      const res = await fetch('/api/customers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete_customer', id: Number(id) })
      });
      const data = await res.json();
      if (data.ok) {
        this.toast(`Pelanggan "${name}" berhasil dihapus`, 'success');
        this.renderPelanggan();
      } else {
        this.toast(data.msg || 'Gagal menghapus pelanggan', 'error');
      }
    } catch {
      this.toast('Gagal menghubungi server', 'error');
    }
  ...
  openDeliveryModalForOrder(orderCode, name, phone, address) {
    this.navigate('/delivery');
    setTimeout(() => {
      this.openDeliveryModal();
      const codeInp = document.getElementById('dlvOrderCode');
      const nameInp = document.getElementById('dlvName');
      const phoneInp = document.getElementById('dlvPhone');
      const addrInp = document.getElementById('dlvAddress');
      const typeSel = document.getElementById('dlvType');
      if (codeInp) codeInp.value = orderCode || '';
      if (nameInp) nameInp.value = name || '';
      if (phoneInp) phoneInp.value = phone || '';
      if (addrInp) addrInp.value = address || '';
      if (typeSel) typeSel.value = 'delivery';
    }, 200);
  },
  ...
  async applyReportFilter() {
    ...
    if (!this._reportPreset || this._reportPreset === 'all') {
      if (start || end) this._reportPreset = 'custom';
    }
  ...
  toggleNotifDropdown() {
    ...
    if (btn) btn.setAttribute('aria-expanded', isShown ? 'false' : 'true');
    if (!isShown) {
      this.updateNotifications();
    }
  },
  ...
  exportCustomersCsv() {
    const custs = this._customers || [];
    if (!custs.length) { this.toast('Tidak ada data pelanggan', 'warning'); return; }
    const headers = ['Kode', 'Nama', 'Telepon', 'Email', 'Alamat', 'Tag', 'Jumlah Pesanan', 'Total Belanja (Rp)'];
    ...
    a.href = url;
    a.download = `Pelanggan_Embun_${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    this.toast('Data pelanggan berhasil diekspor', 'success');
  },
  ...
  _updateBatchBar() {
    ...
    if (checked.length > 0) {
      ...
      if (cnt) cnt.textContent = `${checked.length} dipilih`;
    } else {
      bar.style.display = 'none';
    }
  },

  async _applyBatchStatus() {
    const sel = document.getElementById('batchStatus');
    ...
    this.toast(`${ok} berhasil${fail ? `, ${fail} gagal` : ''}`, ok ? 'success' : 'error');
    this.renderPesanan();
  },

  exportOrdersCsv() {
    const orders = this._orders || [];
    if (!orders.length) { this.toast('Tidak ada data pesanan', 'warning'); return; }
    const headers = ['Kode', 'Pelanggan', 'Telepon', 'Layanan', 'Berat (kg)', 'Total (Rp)', 'Status', 'Pembayaran', 'Tanggal'];
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `Pesanan_Embun_${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    this.toast('Data pesanan berhasil diekspor', 'success');
  },

  _handleDeepLink() {
    const params = new URLSearchParams(window.location.search);
    if (params.get('new_order') === '1') {
      // Navigate to pesanan page and auto-open order modal
      this.navigate('/pesanan');
      setTimeout(() => {
        const btn = document.getElementById('openNewOrderModal');
        if (btn) btn.click();
        // Pre-fill service & weight from landing page
        const svcId = params.get('service_id');
        const kg = params.get('kg');
        if (svcId) {
          const sel = document.getElementById('ordService');
          if (sel) { sel.value = svcId; sel.dispatchEvent(new Event('change')); }
        }
        if (kg) {
          const inp = document.getElementById('ordWeight');
          if (inp) { inp.value = kg; inp.dispatchEvent(new Event('input')); }
        }
  ...
  exportReportCsv() {
    const data = this._lastReportData;
    const daily = (data && data.daily) || [];
    if (!daily.length) {
      this.toast('Tidak ada data transaksi harian untuk diekspor', 'warning');
      return;
    }

    const headers = ['Tanggal', 'Jumlah Order', 'Total Berat (kg)', 'Total Pendapatan (Rp)'];
    const rows = daily.map(d => [
      `"${String(d.d || '').replace(/"/g, '""')}"`,
      Number(d.orders || 0),
      Number(d.weight || 0),
      Number(d.revenue || 0)
    ]);
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    const filter = this._lastReportFilter || {};
    const dateStr = filter.start ? `${filter.start}_sd_${filter.end || 'kini'}` : new Date().toISOString().slice(0, 10);
    a.href = url;
    a.download = `Laporan_Keuangan_Embun_${dateStr}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    this.toast('Laporan CSV berhasil diunduh', 'success');
  },
        ...
        <div class="card" style="padding: 18px 22px; border-radius: var(--radius-md); margin-bottom: 24px;">
          <div style="display: flex; flex-wrap: wrap; gap: 14px; align-items: flex-end; justify-content: space-between;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 6px; align-items: center;">
              <button type="button" class="cat-tab ${this._reportPreset === 'month' ? 'active' : ''}" onclick="App.setReportPreset('month')" style="padding: 6px 14px; font-size: 12px;">Bulan Ini</button>
              <button type="button" class="cat-tab ${this._reportPreset === '30days' ? 'active' : ''}" onclick="App.setReportPreset('30days')" style="padding: 6px 14px; font-size: 12px;">30 Hari</button>
              <button type="button" class="cat-tab ${this._reportPreset === 'year' ? 'active' : ''}" onclick="App.setReportPreset('year')" style="padding: 6px 14px; font-size: 12px;">Tahun Ini</button>
              <button type="button" class="cat-tab ${this._reportPreset === 'all' || !this._reportPreset ? 'active' : ''}" onclick="App.setReportPreset('all')" style="padding: 6px 14px; font-size: 12px;">Semua</button>
              <button type="button" class="btn btn-sm" onclick="App.exportReportCsv()" style="padding: 6px 14px; font-size: 12px; background: var(--green, #10b981); color: #ffffff; border-radius: 99px; font-weight: 600; cursor: pointer; border: none; display: inline-flex; align-items: center; gap: 4px;">📥 Ekspor CSV</button>
        ...
        svg.querySelectorAll('.chart-col').forEach(col => {
          col.addEventListener('mouseenter', () => {
            ...
            tooltip.innerHTML = `
              ...
              ${/^\d{4}-\d{2}-\d{2}$/.test(period) ? `<div style="font-size: 10px; color: rgba(255,255,255,0.6); margin-top: 6px; text-align: center;">💡 Klik bar untuk lihat rincian pesanan</div>` : ''}
            `;
          ...
          col.addEventListener('click', () => {
            ...
            if (/^\d{4}-\d{2}-\d{2}$/.test(period)) {
              this.navigate('/pesanan');
              setTimeout(() => {
                this.renderPesanan({ start: period, end: period });
                this.toast(`Rincian transaksi tanggal ${period}`, 'info');
              }, 150);
            }
          });
    ...
    try {
      ...
      c.innerHTML = `
        <div style="max-width: 960px; margin: 0 auto;">
          ...
          <!-- ROW 2: SYSTEM PREFERENCES & SETTINGS -->
          <div class="card" style="padding: 26px 28px; border-radius: var(--radius-md); margin-top: 24px;">
            <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; margin-bottom: 20px;">
              <div>
                <div style="display: flex; align-items: center; gap: 10px;">
                  <span style="font-size: 20px;">⚙️</span>
                  <h3 style="margin: 0; font-size: 17px; font-weight: 800; color: var(--text);">Pengaturan &amp; Preferensi Sistem</h3>
                </div>
                <p style="margin: 4px 0 0; font-size: 13px; color: var(--muted);">Sesuaikan tampilan aplikasi, bahasa, dan opsi notifikasi.</p>
              </div>
              <span class="badge status-selesai" style="font-size: 12px; padding: 4px 10px;">Cloud POS v2.5 · TiDB Online</span>
            </div>

            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 18px;">
              <!-- TEMA -->
              <div style="padding: 16px; border-radius: 10px; background: var(--bg); border: 1px solid var(--line);">
                <div style="font-weight: 700; font-size: 13px; color: var(--text); margin-bottom: 4px;">Tema Tampilan</div>
                <div style="font-size: 12px; color: var(--muted); margin-bottom: 12px;">Pilih palet warna yang nyaman untuk mata Anda.</div>
                <div style="display: flex; gap: 8px;">
                  <button type="button" class="btn btn-sm" onclick="App.applyTheme('light');localStorage.setItem('theme','light');App.updateThemeIcons(false);App.renderProfile();" style="flex:1;padding:8px;font-size:12px;border:1px solid var(--line);background:${document.documentElement.getAttribute('data-theme')==='dark'?'var(--card)':'var(--blue)'};color:${document.documentElement.getAttribute('data-theme')==='dark'?'var(--text)':'#fff'};border-radius:8px;font-weight:700;">☀️ Terang</button>
                  <button type="button" class="btn btn-sm" onclick="App.applyTheme('dark');localStorage.setItem('theme','dark');App.updateThemeIcons(true);App.renderProfile();" style="flex:1;padding:8px;font-size:12px;border:1px solid var(--line);background:${document.documentElement.getAttribute('data-theme')==='dark'?'var(--blue)':'var(--card)'};color:${document.documentElement.getAttribute('data-theme')==='dark'?'#fff':'var(--text)'};border-radius:8px;font-weight:700;">🌙 Gelap</button>
                </div>
              </div>

              <!-- BAHASA -->
              <div style="padding: 16px; border-radius: 10px; background: var(--bg); border: 1px solid var(--line);">
                <div style="font-weight: 700; font-size: 13px; color: var(--text); margin-bottom: 4px;">Bahasa / Language</div>
                <div style="font-size: 12px; color: var(--muted); margin-bottom: 12px;">Pilihan lokalisasi teks antarmuka.</div>
                <div style="display: flex; gap: 8px;">
                  <button type="button" class="btn btn-sm" onclick="App.applyLang('id');App.renderProfile();" style="flex:1;padding:8px;font-size:12px;border:1px solid var(--line);background:${this.lang==='en'?'var(--card)':'var(--blue)'};color:${this.lang==='en'?'var(--text)':'#fff'};border-radius:8px;font-weight:700;">🇮🇩 Indonesia</button>
                  <button type="button" class="btn btn-sm" onclick="App.applyLang('en');App.renderProfile();" style="flex:1;padding:8px;font-size:12px;border:1px solid var(--line);background:${this.lang==='en'?'var(--blue)':'var(--card)'};color:${this.lang==='en'?'#fff':'var(--text)'};border-radius:8px;font-weight:700;">🇬🇧 English</button>
                </div>
              </div>

              <!-- AUDIO NOTIFIKASI -->
              <div style="padding: 16px; border-radius: 10px; background: var(--bg); border: 1px solid var(--line);">
                <div style="font-weight: 700; font-size: 13px; color: var(--text); margin-bottom: 4px;">Umpan Balik Suara (Chime)</div>
                <div style="font-size: 12px; color: var(--muted); margin-bottom: 12px;">Nada audio harmonik saat transaksi sukses.</div>
                <button type="button" class="btn btn-sm" onclick="App.playSuccessChime();App.toast('Contoh nada audio sukses!','success');" style="width:100%;padding:8px;font-size:12px;border:1px solid var(--line);background:var(--card);color:var(--text);border-radius:8px;font-weight:700;">
                  🔔 Uji Nada Audio (Chime)
                </button>
              </div>
            </div>
          </div>
